...
void notificationExample(std::ostream& out, long port, const AmsAddr& server)
{
    ...
    const long addStatus = AdsSyncAddDeviceNotificationReqEx(port,
                                                             ...
                                                             &hNotify);
    if (addStatus) {
        out << "Add aa device notification failed with: " << std::dec << addStatus << '\n';
        return;
    }
...
void notificationByNameExample(std::ostream& out, long port, const AmsAddr& server)
{
    ...
    const long addStatus = AdsSyncAddDeviceNotificationReqEx(port,
                                                             ...
                                                             &hNotify);
    if (addStatus) {
        out << "Add bb device notification failed with: " << std::dec << addStatus << '\n';
        return;
    }
...
void readExample(std::ostream& out, long port, const AmsAddr& server, uint32_t offset)
{
    uint32_t bytesRead;
    uint32_t buffer;
    ...
    for (size_t i = 0; i < 2; ++i) {
        const long status = AdsSyncReadReqEx2(port, &server, 0x4020, offset, sizeof(buffer), &buffer, &bytesRead);
        if (status) {
...
void runADS(std::ostream& out, uint32_t offset)
{
    static const AmsNetId remoteNetId { 5, 19, 209, 26, 1, 1 };
    //static const char remoteIpV4[] = "169.254.227.236";	
	//static const char remoteIpV4[] = "127.0.0.1";
	static const char remoteIpV4[] = "10.0.0.2";
	//static const char remoteIpV4[] = "192.168.2.119";

	AdsSetLocalAddress({ 192, 168, 2, 41, 1, 2 });
	
	//static const char remoteIpV4[] = "10.0.0.2";
	//AdsSetLocalAddress({ 10, 0, 0, 1, 1, 2 });
	...
	/*
	out << "before retun\n";
	AdsDelRoute(remoteNetId); return;
	out << "after return\n"; */
    ...
    readExample(out, port, remote, offset);
